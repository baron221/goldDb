      ...
      <el-col :span="6">
        <el-card shadow="never">
          ...
          >
            ...
            <el-table-column prop="ManufacturerCount" label="업체수" width="70" align="center">
              <template #default="scope">
                <el-tag size="small" type="info">{{ scope.row.ManufacturerCount }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column
              label="지역"
...
import { getCompanies, getManufacturersByCenter, getUnassignedManufacturers, assignManufacturersToCenter } from '@/api/company';
...
const fetchMappingData = async (centerId) => {
  ...
  try {
    const [assignedRes, unassignedRes] = await Promise.all([
      getManufacturersByCenter(centerId),
      getUnassignedManufacturers(centerId)
    ]);

    assignedManufacturers.value = assignedRes.data;
    unassignedManufacturers.value = unassignedRes.data;
  } catch (error) {
...
const filteredUnassigned = computed(() => {
  return unassignedManufacturers.value.filter(r => {
...
const filteredAssigned = computed(() => {
  return assignedManufacturers.value.filter(r =>
...
const moveToAssigned = async () => {
  ...
  const newAssignedIds = [...assignedManufacturers.value.map(r => r.id), ...moveIds];

  mappingLoading.value = true;
  try {
    await assignManufacturersToCenter(selectedCenter.value.id, newAssignedIds);
...
const moveToUnassigned = async () => {
  ...
  const newAssignedIds = assignedManufacturers.value
    .map(r => r.id)
