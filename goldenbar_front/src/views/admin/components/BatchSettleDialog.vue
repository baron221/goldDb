...
const props = defineProps({
  ...
  // against an already-charged, already-touched balance, not a fresh sale. Every order this
  // targets is by definition already showing in 미수금 관리 (it got there by having ≥1
  // application on file already), so its product/order info is old news here and only
  // clutters what should read as "paying down a balance" - hide the 주문내용/purity tables
  // and the order-count wording so this reads as a plain debt payment.
  hideProducts: {
    type: Boolean,
    default: false
  }
...
const handleSubmit = async () => {
  // Guards the whole flow, not just the API call - see ReceivableBatchSettleDialog's
  // identical fix. Engaging :loading only inside the confirm's .then() left a window where
  // a fast double-click could stack two confirm dialogs before either resolved, each going
  // on to call processPayment independently.
  ...
  try {
    await ElMessageBox.confirm(
      props.hideProducts
        ? `${summary.value.companyName}에 ₩${formatPrice(settleForm.amount)} 입금 처리하시겠습니까?`
