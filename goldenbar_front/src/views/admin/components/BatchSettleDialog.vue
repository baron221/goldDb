...
<base-popup v-model="visible" title="정산 처리" width="620px" @close="handleClose">
    <div v-if="summary" style="margin-bottom: 1.25rem; padding: 0.9375rem; border-radius: 2px; background: #fafafa;">
      ...
      <div style="font-size: 0.875rem; color: #606266;">{{ singleMode ? '단일 주문 정산' : `선택된 주문 ${orderIds.length}건` }}</div>
    </div>
    ...
    <table class="ledger-table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="ledger-readonly">₩ {{ formatPrice(summary?.saleAmount || 0) }}</td>
        </tr>
        <tr>
          ...
          <td>
            <el-input-number v-model="settleForm.amount" :min="0" :max="singleMode ? (summary?.saleAmount || 0) : undefined" :step="1000" size="small" style="width: 100%;" />
          </td>
        </tr>
        <tr>
...
const afterAmount = computed(() => {
  ...
  const b = summary.value?.saleAmount || 0;
...
const fetchSummary = async () => {
  if (!props.orderIds || props.orderIds.length === 0) return;
  loading.value = true;
  try {
    const res: any = await getOrderChargeSummary(props.orderIds);
...
const handleSubmit = () => {
  ...
  if (settleForm.amount <= 0 && settleForm.weight <= 0 && settleForm.discount <= 0 && settleForm.discountWeight <= 0) {
    ElMessage.error('결제액, 할인액 또는 중량 중 하나는 0보다 커야 합니다.');
    return;
  }

  ElMessageBox.confirm(
    `${summary.value.companyName}에 ₩${formatPrice(settleForm.amount)} 정산 처리하시겠습니까? (주문 ${props.orderIds.length}건)`,
    '정산 처리 확인',
