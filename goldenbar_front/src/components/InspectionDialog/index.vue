...
const initializeForm = () => {
  ...
  props.order.orderItems.forEach((item: any) => {
    ...
    items.push({
      ...
      retailerConfirmLaborCost: Math.round((item.factoryInputLaborCost || 0) * 2 * ((item.settlementRatio || 70) / 100)),
    ...
    if (item.children && item.children.length > 0) {
      item.children.forEach((child: any) => {
        items.push({
          ...
          retailerConfirmLaborCost: Math.round((child.factoryInputLaborCost || 0) * 2 * ((child.settlementRatio || 70) / 100)),
...
const handleRatioChange = (row: any) => {
  ...
  row.retailerConfirmLaborCost = Math.round(factoryLabor * 2 * (ratio / 100));
};
